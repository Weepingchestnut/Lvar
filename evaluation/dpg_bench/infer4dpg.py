...
if __name__ == '__main__':
    ...
    for index, row in tqdm(prompts_df.iterrows(), total=len(prompts_df)):
        ...
        if os.path.exists(save_file):
            print(f"Skipping [{item_id}], file already exists.")
            continue

        print(f"\nProcessing ({index+1: >4}/{len(prompts_df)}): item_id='{item_id}'")
        ...
        if len(images) == args.n_samples:
            print(f"  Gridding {len(images)} images into a 2x2 format...")
            ...
            print(f"  Saved grid image to: {save_file}")
        else:
            print(f"Warning: Expected {args.n_samples} images but got {len(images)}. \
