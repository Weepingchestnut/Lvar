...
from models.ema import get_ema_model
...
# InfinityStar
# =============
def load_video_visual_tokenizer(args, device=None):
    if not device:
    ...
    if args.vae_type in [8,12,14,16,18,20,24,32,48,64,128]:
        ...
        if args.videovae == 10: # absorb patchify
            from models.videovae.models.load_vae_bsq_wan_absorb_patchify import video_vae_model
...
def build_video_vae_gpt(args: arg_util.Args, force_flash=False, device='cuda'):
    vae_local = load_video_visual_tokenizer(args, device)
    ...
    from timm.models import create_model
    gpt_wo_ddp: InfinityStar = create_model(model_str, **gpt_kw)
