...
from models.hart.hart_transformer_t2i import HARTForT2I         #! don't remove, for model register
from tools.run_hart import default_prompts, encode_prompts, is_dangerous, llm_system_prompt
...
def save_images(sample_imgs, sample_folder_dir, store_separately, prompts):
    if not store_separately and len(sample_imgs) > 1:
        ...
        # bs, 3, r, r
        print(f'{type(sample_imgs)=}, {sample_imgs.shape=}')
        if sample_imgs.dtype == torch.float16:
            sample_imgs_np = sample_imgs.mul_(255).cpu().numpy()
        else:
            sample_imgs_np = sample_imgs.to(torch.float32).mul_(255).cpu().numpy()
        ...
        for img_idx in range(num_imgs):
            ...
            save_path = os.path.join(sample_folder_dir, f"{img_idx:06d}_{time_str()}.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="pretrained_models/hart/shieldgemma-2b",
    )
    parser.add_argument(
        "--prompt", type=str, help="A single prompt.",
        default="An astronaut riding a horse on the moon, oil painting by Van Gogh."
        # default='three women'
        # default='A pencil lies on the white paper.'
        # default='A beautiful Chinese woman with graceful features, close-up portrait, long flowing black hair, wearing a traditional silkcheongsam delicately embroidered with floral patterns, face softly illuminated by ambient light, serene expression.'
    )
    ...
    parser.add_argument(
        ...
        default="work_dir/play_models/hart/samples/",
    )
    parser.add_argument(
        "--store_seperately",
