...
def build_everything(args: arg_util.Args):
    ...
    if not args.local_debug:
        ...
        ld_val = DataLoader(
            ...
            batch_size=round(args.batch_size*1.5),          # maybe for Mixed Precision Training?
    ...
    if trainer_state is not None and len(trainer_state):
        trainer.load_state_dict(trainer_state, strict=False, skip_vae=True)     # do not load vae again
    ...
    if args.local_debug:
        ...
        rng.manual_seed(0)
        B = 4
        inp = torch.rand(B, 3, args.data_load_reso, args.data_load_reso).to('cuda')
        label = torch.ones(B, dtype=torch.long).to('cuda')
...
def main_training():
    ...
    if args.local_debug:
        torch.autograd.set_detect_anomaly(True)     # 用于调试梯度计算的工具
