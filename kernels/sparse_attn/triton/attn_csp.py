...
def _sparse_attn_fwd_inner(acc, l_i, m_i, q,  #
                    ...
                    stride_spi_group, stride_spi_col, stride_spc_group, #
                    sparsity_indices_ptr, sparsity_counts_ptr, #
                    ):
    sparsity_count = tl.load(sparsity_counts_ptr + indices_group * stride_spc_group)
    # sparsity_count = tl.load(sparsity_counts_ptr + start_m)
    # sparsity_count = 0
    sparsity_offsets = tl.arange(0, BLOCK_N)
    sparsity_indices_ptr += indices_group * stride_spi_group + sparsity_offsets * stride_spi_col
    ...
    for start_n in range(0, sparsity_count, BLOCK_N):
        ...
        sparsity_indices = tl.load(sparsity_indices_ptr, mask=valid_sparse, other=0)
        ...
        qk = tl.dot(q, k)
        # Sparse columns are KV positions; mask partial sparse-count tails before
        ...
        acc = tl.dot(p, v, acc)
        m_i = m_ij
        sparsity_indices_ptr += BLOCK_N * stride_spi_col
        cur_iter += 1
...
def _sparse_attn_fwd(Q, K, V, sm_scale, M, L, Out, Out_accum, Out_scale: tl.constexpr, #
    ...
    acc, l_i, m_i = _sparse_attn_fwd_inner(acc, l_i, m_i, q, K_block_ptr, V_block_ptr,  #
                                    ...
                                    stride_kn, stride_vk, #
                                    stride_spim, stride_spin, stride_spcm, #
                                    spi_ptr, spc_ptr, #
                                    )
    # epilogue
