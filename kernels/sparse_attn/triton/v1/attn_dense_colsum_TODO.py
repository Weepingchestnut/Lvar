...
def _full_attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    # 1. 加载上一步的 Softmax 统计量 (prev_maxes, prev_norm)
    # 这些统计量用于近似计算当前的 Attention Probabilities，从而计算 Column Sums
    ...
    for start_n in range(0, N_CTX_K, BLOCK_N):
        ...
        # Online Softmax update (compute max, sum, update acc)
        m_ij = tl.maximum(m_i, tl.max(qk, 1) * qk_scale)
        # qk = qk * qk_scale - m_ij[:, None]
        qk = qk * qk_scale - m_ij[:, None]
        
        # Mask KV if out of bounds
        if should_mask_kv:
            qk = tl.where(start_n + offs_n[None, :] < N_CTX_K, qk, -1.0e6)
        p = tl.math.exp2(qk)
        l_ij = tl.sum(p, 1)
        ...
        # 4. 对 Query 块内求和 (Reduction over BLOCK_M)
        blocksums = tl.sum(p_prev, 0)
        
        # 5. 原子加 (Atomic Add) 到全局显存
        # 因为多个 Query Block 会贡献给同一个 Key Block，所以需要原子加
        tl.atomic_add(bsp, blocksums, mask=(start_n + offs_n) < N_CTX_K, sem='relaxed')
...
@triton.autotune(list(filter(keep, configs)), key=["N_CTX_Q", "N_CTX_K", "HEAD_DIM"])
@triton.jit
def _full_attn_fwd(Q, K, V, sm_scale, M, L, Out,  #
    ...
    acc, l_i, m_i = _full_attn_fwd_inner(
        ...
        N_CTX_Q, N_CTX_K, 
        V.dtype.element_ty == tl.float8e5, should_mask_kv  #
    )
    # epilogue
...
def main():
    ...
    # Define shapes for Cross-Attention (Q != K)
    # Batch=1, Head=24, Dim=128
    # Q_Len = 100, K_Len = 200 (Arbitrary different lengths)
    batch = 1
    heads = 24
    dim = 128
    q_len = 100
    ...
    q_shape = (batch, heads, q_len, dim)
    k_shape = (batch, heads, k_len, dim)
    
    q = torch.randn(q_shape)
    k = torch.randn(k_shape)
    v = torch.randn(k_shape) # V must match K length

    # Fake previous LSE for testing
    # Shape must match [Batch, Head, Q_Len]
    prev_maxes = torch.zeros((batch, heads, q_len, 1), dtype=torch.float32)
    prev_norm = torch.ones((batch, heads, q_len, 1), dtype=torch.float32)
    prev_lse = (prev_maxes, prev_norm)

    # Run modified kernel
    o, blocksums, new_lse = dense_colsum_attn(q, k, v, prev_lse)
    
    # Run reference (PyTorch standard attention)
    o_ref = F.scaled_dot_product_attention(q, k, v)

    # Check shapes
    print(f"Output shape: {o.shape}, Reference shape: {o_ref.shape}")
    print(f"Blocksums shape: {blocksums.shape}") # Should be [1, 24, ceil(100/192), 200]
    
    # Check correctness
    assert torch.allclose(o, o_ref, atol=1e-2, rtol=1e-2), "Output mismatch!"
    print("Test Passed: Output matches PyTorch reference.")
