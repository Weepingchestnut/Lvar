...
from models.infinity.infinity_model import (
    MultiInpIdentity, MultipleLayers, SharedAdaLin,
...
class SparseVAR_Infinity(nn.Module):
    def __init__(
        self, vae_local,
    ...
    def lf_anchor(
        ...
        upsampled_logits = torch.nn.functional.interpolate(
            logits_BlV.reshape(B, -1, self.patch_nums[si-1], self.patch_nums[si-1]),
            size=(patch_size, patch_size), mode='nearest'
        )
        upsampled_mse = torch.nn.functional.interpolate(
            mse_difference.reshape(B, 1, self.patch_nums[si-1], self.patch_nums[si-1]),
            size=(patch_size, patch_size), mode='nearest'
        )
        ...
        anchor_candidates = generate_anchors_from_flat(patch_size, local_window_size).to(logits_BlV.device)
        anchor_candidates = anchor_candidates.unsqueeze(0).expand(B, -1, -1)
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            for block_idx, b in enumerate(self.block_chunks):
                ...
                if self.add_lvl_embeding_only_first_block and block_idx == 0:
                    ...
                    if si>=start_prune_stage and compress_method=="sparsevar":
                        if compress_method=="sparsevar":
                            hr_idx,lr_idx,recover_lr,anchor_lr=self.lf_anchor(logits_BlV_copy,si,mse_difference,compress_ratio_,local_window_size)
                ...
                for m in b.module:
                    if hr_idx is not None:
                        ...
                        seq_len=None
                    last_stage,_ = m(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si,hr_idx=hr_idx,seq_len=seq_len)
                ...
                if si>=start_prune_stage-1:
                    if block_idx==specific_mse_layer and compress_method=="sparsevar":
                        if hr_idx is not None:
                        ...
                        mse_difference=torch.stack([mse_difference[:B],mse_difference[B:]]).mean(0)
                        if return_features:
                            mse_differences.append(mse_difference)
                        
                        compress_ratio_=((mse_difference/mse_difference.max())>compress_ratio).float().mean(1).max()
