...
@register_model
def fastvar_infinity_2b(depth=32, embed_dim=2048, num_heads=2048//128, drop_path_rate=0.1, mlp_ratio=4, block_chunks=8, **kwargs):
    return FastVAR_Infinity(
        depth=depth,
        embed_dim=embed_dim,
        num_heads=num_heads,
        mlp_ratio=mlp_ratio,
...
@register_model
def fastvar_infinity_8b(depth=40, embed_dim=3584, num_heads=3584//128, drop_path_rate=0.1, mlp_ratio=4, block_chunks=8, **kwargs):
    return FastVAR_Infinity(
        depth=depth,
        embed_dim=embed_dim,
        num_heads=num_heads,
...
class FastVAR_Infinity(Infinity):
    def __init__(self, *args, **kwargs):
        ...
        cached_scale = getattr(self.other_args, 'cached_scale', 8)
        prune_ratio = getattr(self.other_args, 'prune_ratio', '0.4,0.5')
        prune_ratio = tuple([float(item) for item in prune_ratio.split(',')])
        ...
        for block_idx in range(self.depth):
            block = (FastVARCrossAttnBlock if self.t2i else SelfAttnBlock)(
                embed_dim=self.C,
                kv_dim=self.D,
                cross_attn_layer_scale=self.cross_attn_layer_scale,
                cond_dim=self.D,
                act=True,
        ...
        print(
            f'    cached scale is {cached_scale}\n',
            f'    pruning ratio {prune_ratio}'
        )
...
if __name__ == '__main__':
    ...
    with autocast("cuda", dtype=torch.bfloat16, enabled=True, cache_enabled=True), torch.no_grad():
        infinity_test: FastVAR_Infinity = FastVAR_Infinity(
            ...
            fused_norm=True,                # ! FastVAR default is True
