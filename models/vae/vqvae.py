...
from models.vae.basic_vae import Decoder, Encoder
...
class VQVAE(nn.Module):
    ...
    def forward(self, inp, ret_usages=False):   # -> rec_B3HW, idx_N, loss
        ...
        return (
            self.decoder(self.post_quant_conv(f_hat)),
            usages,
            vq_loss,
            # mean_entropy_loss,      # ? HMAR add, but not match `VectorQuantizer2.forward` return
        )
    # ===================== `forward` is only used in VAE training =====================
    ...
    def img_to_idxBl(
        ...
        f = self.quant_conv(self.encoder(inp_img_no_grad))      # encoder: [bs, 3, 256, 256] -> [bs, 32, 16, 16] --quant_conv--> [bs, 32, 16, 16]
