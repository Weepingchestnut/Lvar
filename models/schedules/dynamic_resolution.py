...
def get_ratio2hws_video_v2():
    ...
    for h_div_w in [1, 100/116, 3/4, 2/3, 9/16, 1/2, 2/5, 1/3]:     # 720p: 720/1280 = 9/16
        scale_schedule = []
...
def get_first_full_spatial_size_scale_index(vae_scale_schedule):
    """Find the first scale index whose spatial resolution has reached the final size (H_final, W_final).

    Args:
        vae_scale_schedule (list): 480p e.g., [(1, 1, 1), (1, 2, 3), ..., (1, 30, 53), (20, 1, 1), ..., (20, 30, 53)]

    Returns:
        int: the boundary scale between image pyramid and video clip pyramid
    """
...
def get_ratio2hws_pixels2scales(dynamic_scale_schedule, video_frames):
    ...
    if dynamic_scale_schedule == '13_hand_craft':
        ...
        # 1. Generate a basic spatial scale schedule
        ratio2hws, total_pixels2scales = get_ratio2hws_video_v2()
        # 2. Unify into (1, h, w)
        ratio2hws = append_dummy_t(ratio2hws)
        # 3. Generate a full scaling schedule according to different ratios and different pn
        dynamic_resolution_h_w = get_full_ratio2hws(ratio2hws, video_frames, total_pixels2scales, predefined_t=None)

        compressed_frames_in_one_clip = 20      # one video clip pyramid = 20 compressed frames
        compressed_frames_per_sec = 16 // 4     # video is 16fps, temporal compression ratio is 4 ==> per-second corresponds to 4 compressed frames
        duration_resolution = 1

        for ratio in dynamic_resolution_h_w:
            for pn in dynamic_resolution_h_w[ratio]:
                # build full spatial_time_schedule
                # if ratio == 0.562 and pn == '0.90M':
                #     print('debug for 720p')
                base_scale_schedule = dynamic_resolution_h_w[ratio][pn]['scales']
                ...
                for pt in range(1+compressed_frames_per_sec,compressed_frames+1, pt_interval): # duration_resolution is 1s      pt: 1, 5, 9, 13, 17, 21
                    tmp_clips = 1 + int(np.ceil((pt-1) / compressed_frames_in_one_clip))        # current clip number
...
def grn_get_ratio2hws_pixels2scales(dynamic_scale_schedule, train_h_div_w_list, video_frames):
    compressed_frames = video_frames // 4 + 1
...
def get_full_ratio2hws(ratio2hws, video_frames, total_pixels2scales, predefined_t=None):
    ...
    for ratio, hws in ratio2hws.items():
        ...
        full_ratio2hws[int(real_ratio*1000)/1000] = hws     # save 3 decimal places, e.g. 0.8615384615384616 --> 0.861
        if ratio != 1.000:      # add w/h ratio, (t, h, w) --> (t, w, h)
...
def grn_get_dynamic_resolution_meta(dynamic_scale_schedule, train_h_div_w_list, video_frames):
    dynamic_resolution_h_w = grn_get_ratio2hws_pixels2scales(dynamic_scale_schedule, train_h_div_w_list, video_frames)
    h_div_w_templates = []
    for h_div_w in dynamic_resolution_h_w.keys():
        h_div_w_templates.append(h_div_w)
    h_div_w_templates = np.array(h_div_w_templates)
